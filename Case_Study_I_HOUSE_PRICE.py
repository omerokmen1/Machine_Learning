...
# İş Problemi: Elimizdeki veri seti üzerinden minimum hata ile ev fiyatlarını tahmin eden bir makine öğrenmesi modeli
# geliştiriniz ve kaggle yarışmasına tahminlerinizi yükleyiniz.
...
# Yüksek korelasyona sahip olan kolonlar
...
def quick_missing_imp(data, num_method = "median", cat_length = 20, target = "SalePrice"):
    ...
    # Değişken object ve sınıf sayısı cat_length'e eşit veya altındaysa boş değerleri mode ile dolduruyor.
    data = data.apply(lambda x: x.fillna(x.mode()[0]) if (x.dtype == "O" and len(x.unique()) <= cat_length) else x, axis=0)

    # num_method mean, tipi object olmayan değişkenlerin boş değerlerini ortalama ile dolduruyor.
    if num_method == "mean":
        data = data.apply(lambda x: x.fillna(x.mean()) if x.dtype != "O" else x, axis=0)

    # num_method median, tipi object olmayan değişkenlerin boş değerlerini ortalama ile dolduruyor.
...
# test dataframe' indeki boş olan salePrice değişkenlerini tahminleyiniz ve Kaggle sayfasına submit etmeye uygun halde
